...
FRESULT chk_lock (  /* Check if the file can be accessed */
    FFDIR* dj,      /* Directory object pointing the file to be checked */
...
UINT inc_lock ( /* Increment file open counter and returns its index (0:int error) */
    FFDIR* dj,  /* Directory object pointing the file to register or increment */
...
static
FRESULT dir_sdi (
    FFDIR *dj,      /* Pointer to directory object */
...
static
FRESULT dir_find (
    FFDIR *dj           /* Pointer to the directory object linked to the file name */
)
{
    FRESULT res;
    BYTE c, *dir;
...
static
FRESULT dir_read (
    FFDIR *dj           /* Pointer to the directory object that pointing the entry to be read */
)
{
    FRESULT res;
    BYTE c, *dir;
...
FRESULT dir_register (  /* FR_OK:Successful, FR_DENIED:No free entry or too many SFN collision, FR_DISK_ERR:Disk error */
    FFDIR *dj           /* Target directory with object name to be created */
)
{
    FRESULT res;
    BYTE c, *dir;
...
FRESULT dir_remove (    /* FR_OK: Successful, FR_DISK_ERR: A disk error */
    FFDIR *dj           /* Directory object pointing the entry to be removed */
)
{
    FRESULT res;
#if _USE_LFN    /* LFN configuration */
...
static
FRESULT create_name (
    FFDIR *dj,          /* Pointer to the directory object */
...
#endif
        if (w < 0x80 && chk_chr("\"*:<>\\?|", w)) /* Reject illegal chars for LFN */
            return FR_INVALID_NAME;
    ...
    *path = &p[si];                     /* Return pointer to the next segment */
    ...
    for (;;) {
        ...
        if (IsDBCS1(c)) {               /* Check if it is a DBC 1st byte (always false on SBCS cfg) */
            ...
            if (chk_chr("\"*+,:;<=>\\?[]|", c)) /* Reject illegal chrs for SFN */
                return FR_INVALID_NAME;
...
FRESULT follow_path (   /* FR_OK(0): successful, !=0: error code */
    FFDIR *dj,          /* Directory object to return last directory and found object */
...
FRESULT f_opendir (
    FFDIR *dj,          /* Pointer to directory object to create */
    const TCHAR *path   /* Pointer to the directory path */
)
{
    FRESULT res;
        ...
        if (res == FR_OK) {                 /* The object is accessible */
            ...
            if (res == FR_OK && (dir[DIR_Attr] & AM_DIR)) { /* Is it a sub-dir? */
                if (dclst < 2) {
                    ...
                    mem_cpy(&sdj, &dj, sizeof(FFDIR));  /* Check if the sub-dir is empty or not */
                    sdj.sclust = dclst;
